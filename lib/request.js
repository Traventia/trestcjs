...
exports.r = function(httpMethod,isHttps,host,port,path,params,data,options,callback){
    ...
    try{
        ...
        return _rerr.errorInRequest(errors.ERROR_EXCEPTION_HTTPOPTIONS+error.message,500,null,callback)
    }
    ...
    try {
        ...
        return _rerr.errorInRequest(errors.ERROR_PARSING_BODY_REQUEST+error.message,500,null,callback)
    }
...
var _executeHttpRequest = function(httpOptions,isHttps,body,options,originalQueryRequest,trace,callback){
    ...
    req.on('error', function(error) {
        if (callbackit){
            callbackit = false;
            return _rerr.errorInRequest(errors.ERROR_CONNECTION_TO_SERVER+error.message,502,trace,callback)
        }
    });
    ...
    if (timeout > 0){
        req.setTimeout(timeout,function () {
            if (callbackit){
                callbackit = false;
                req.abort();
                return _rerr.errorInRequest(errors.ERROR_TIMEOUT_ON_REQUEST,504,trace,callback)
            }
        });
    }
...
var _executeRedirection = function(redirection,originalQueryRequest,trace,callback){
    ...
    if (originalQueryRequest.options.maxRedirects==0){
        return _rerr.errorInRequest(errors.ERROR_INFINITE_REDIRECTIONS,508,trace,callback);
    }
        ...
        switch(options.auth.type){
            case 'basic':
                if (options.auth.username !== undefined && options.auth.password !== undefined ){
                    ...
                    httpOptions.headers['Authorization'] = 'Basic '+new Buffer(options.auth.username+':'+options.auth.password).toString('base64');
                }
                break;
        }
    }
