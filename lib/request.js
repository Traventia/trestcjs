...
exports.r = function(httpMethod,isHttps,host,port,path,params,data,options,callback){
    ...
    if (body == null){
        return _rerr.process(errors.ERROR_PARSING_BODY_REQUEST,500,null,callback)
    }
...
var _executeHttpRequest = function(httpOptions,isHttps,body,options,originalQueryRequest,trace,callback){
    ...
    req.on('error', function(e) {
        if (callbackit){
            callbackit = false;
            return _rerr.process(errors.ERROR_CONNECTION_TO_SERVER,502,trace,callback)
        }
    });
    ...
    if (timeout > 0){
        req.setTimeout(timeout,function () {
            if (callbackit){
                callbackit = false;
                req.abort();
                return _rerr.process(errors.ERROR_TIMEOUT_ON_REQUEST,504,trace,callback)
            }
        });
    }
...
var _executeRedirection = function(redirection,originalQueryRequest,trace,callback){
    ...
    if (originalQueryRequest.options.maxRedirects==0){
        return _rerr.process(errors.ERROR_INFINITE_REDIRECTIONS,508,trace,callback);
    }
